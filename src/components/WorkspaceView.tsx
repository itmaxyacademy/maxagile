  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden h-full bg-brand-bg relative">
      ...
      <div className="px-8 pt-8 pb-4 bg-white/60 backdrop-blur-md border-b border-white shadow-sm flex flex-col shrink-0 z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <h2 className="text-3xl font-extrabold text-brand-text tracking-tight mt-1">{workspaceName}</h2>
      ...
      <div className="flex-1 overflow-auto p-8 h-full relative z-10 custom-scrollbar">
        ...
        {/* TAB WATERFALL */}
        {activeTab === 'waterfall' && (
          <div className="max-w-6xl mx-auto bg-white/60 backdrop-blur-md border border-white rounded-3xl shadow-sm p-8 overflow-x-auto">
            <h3 className="font-bold text-brand-text mb-6">Gantt Chart / Tahapan (Waterfall)</h3>
            <div className="min-w-[800px]">
              <div className="flex border-b border-gray-200 pb-2 mb-4">
                <div className="w-1/4 font-semibold text-xs text-gray-500 uppercase">Tahap / Tugas</div>
                <div className="w-3/4 flex text-xs text-gray-400">
                  <div className="flex-1 border-l border-gray-200 pl-2">Kuartal 1 (Jan-Mar)</div>
                  <div className="flex-1 border-l border-gray-200 pl-2">Kuartal 2 (Apr-Jun)</div>
                  <div className="flex-1 border-l border-gray-200 pl-2">Kuartal 3 (Jul-Sep)</div>
                  <div className="flex-1 border-l border-gray-200 pl-2">Kuartal 4 (Okt-Des)</div>
                </div>
              </div>
              
              {['Analisis Kebutuhan', 'Desain Sistem', 'Implementasi', 'Pengujian', 'Penyebaran'].map((phase, idx) => (
                <div key={phase} className="flex mb-4 items-center">
                  <div className="w-1/4 text-sm font-medium text-brand-text">{phase}</div>
                  <div className="w-3/4 relative h-10 bg-white/40 rounded-xl border border-white shadow-sm">
                    {workspaceData.workItems[idx] ? (
                      <div 
                        onClick={() => setSelectedTask(workspaceData.workItems[idx])}
                        className="absolute top-1.5 bottom-1.5 rounded-lg bg-brand-orange text-white text-xs flex items-center px-3 font-semibold shadow-sm cursor-pointer hover:bg-orange-500 transition-all hover:scale-[1.01]"
                        style={{ left: `${idx * 15}%`, right: `${100 - ((idx * 15) + 30)}%` }}
                      >
                        <span className="truncate">{parseStoryPoints(workspaceData.workItems[idx].title).cleanTitle}</span>
                      </div>
                    ) : (
                      <div className="absolute top-1.5 left-[15%] text-gray-400 text-xs flex items-center h-7 font-medium italic">
                        Belum ada tugas untuk fase ini
                      </div>
                    )}
                  </div>
                </div>
